...
void addNewSnake(snake **s, int *snake_capacity) {
    ...
    if (CURRENT_LENGTH > 0) {
        (*s)[CURRENT_LENGTH].x = (*s)[CURRENT_LENGTH - 1].x + (*s)[CURRENT_LENGTH - 1].width;
...
int main(int argc, char* argv[]) {
    ...
    TTF_Font *font = TTF_OpenFont("/usr/share/fonts/TTF/M+2NerdFont-Thin.ttf", 24);
    if (font == NULL) {
    ...
    if (textSurface == NULL) {
        ...
        if (textTexture == NULL) {
            ...
            SDL_Rect textRect = {SCREEN_WIDTH - textWidth - 20, 20, textWidth, textHeight};
